export const data = [
    {
      id: 1,
      name: 'Sate Ayam',
      category: 'indo',
      stock: 20,
      image:
        'https://images.unsplash.com/photo-1529563021893-cc83c992d75d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=387&q=80',
      price: 18000,
      description: "Sate ayam adalah makanan khas Indonesia. Sate Ayam dibuat dari daging ayam. Pada umumnya sate ayam dimasak dengan cara dipanggang dengan menggunakan arang dan disajikan dengan pilihan bumbu kacang atau bumbu kecap. Sate ini biasanya disajikan bersama dengan lontong atau nasi."
    },
    {
      id: 2,
      name: 'Chicken Curry',
      category: 'jepang',
      stock: 15,
      image:
        'https://awsimages.detik.net.id/community/media/visual/2019/01/28/1d0cc924-40e6-4ef7-911b-3114ed6df6a3.jpeg?w=700&q=90',
      price: 22000,
      description: "Mungkin banyak orang mengira nasi kari hanya ada di India. Ternyata, nasi kari juga bisa ditemukan di Jepang dan menjadi makanannya yang unik. Menu yang satu  ini terbuat dari daging yang dilumuri tepung dan disiram dengan kuah kari berisi sayuran seperti wortel, kentang, dan bawang bombai."
    },
    {
      id: 3,
      name: 'Ramen',
      category: 'jepang',
      stock: 19,
      image:
        'https://images.unsplash.com/photo-1584739200850-dc2072fdfe04?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1170&q=80',
      price: 35000,
      description: "Ramen Jepang biasanya punya rasa yang lebih lembut. Sementara Segye Korea Ramyun terasa lebih kuat dan biasanya bercita rasa pedas. Selain itu, bumbu dasar untuk kuah kaldu ramen Jepang biasanya dibuat dari kaldu ayam, seafood, atau babi. Ada lima jenis utama ramen Jepang. Di antaranya adalah shoyu yang merupakan kuah dengan bahan dasar kecap. Miso, dengan bahan dasar pasta kedelai. Kemudian ada shio, kuah dengan bahan dasar garam. Lalu ada Tonkotsu, kuah ramen dengan bahan dasar rebusan tulang babi. Dan terakhir adalah ramen kari."
    },
    {
      id: 4,
      name: 'Nasi Goreng',
      category: 'indo',
      stock: 30,
      image:
        'https://images.unsplash.com/photo-1647093953000-9065ed6f85ef?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=435&q=80',
      price: 9000,
      description: "Nasi goreng adalah makanan berupa nasi yang digoreng dan dicampur dalam minyak goreng, margarin, atau mentega. Biasanya ditambah dengan kecap manis, bawang merah, bawang putih, asam jawa, lada dan bahan lainnya; seperti telur, daging ayam, dan kerupuk. Ada pula nasi goreng jenis lain yang dibuat bersama dengan ikan asin yang juga populer di seluruh Indonesia. Nasi goreng juga dikenal sebagai masakan nasional Indonesia. "
    },
    {
      id: 5,
      name: 'Korean Ramyeon',
      category: 'korea',
      stock: 16,
      image:
        'https://www.selasar.com/wp-content/uploads/2020/09/Ramyeon-Makanan-Khas-Korea.jpg',
      price: 42000,
      description: "Ramyeon versi Korea mempunyai tekstur yang lebih tebal dengan kuah bumbu yang terasa lebih kental. Cita rasanya cenderung pedas, tapi ada juga aneka pilihan rasa lainnya seperti gurih atau asin. Saat ini sudah banyak Ramyeon versi instan yang bisa kamu beli di supermarket Indonesia."
    },
    {
      id: 6,
      name: 'Bakso',
      category: 'indo',
      stock: 21,
      image:
        'https://th.bing.com/th/id/OIP.nvIGCSnyGhtZywZ_Ni_wBwHaEv?pid=ImgDet&rs=1',
      price: 12000,
      description: "Bakso (baso) adalah makanan khas Tionghoa Indonesia berupa bola daging yang terbuat dari campuran tepung tapioka dengan daging segar yang digiling. Pengertian ini didasarkan dari asal mula kata “bakso” yang berasal dari bahasa Hokkien yang berarti daging babi giling. Tetapi karena di Indonesia daging babi tidak dimakan oleh mayoritas masyarakat (muslim), akhirnya biasanya bakso dibuat dari gilingan daging sapi, ikan, udang dan ayam."
    },
    {
      id: 7,
      name: 'Tteokbokki',
      category: 'korea',
      stock: 16,
      image:
        'https://images.unsplash.com/photo-1635363638580-c2809d049eee?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1170&q=80',
      price: 28000,
      description: "Ramyeon versi Korea mempunyai tekstur yang lebih tebal dengan kuah bumbu yang terasa lebih kental. Cita rasanya cenderung pedas, tapi ada juga aneka pilihan rasa lainnya seperti gurih atau asin. Saat ini sudah banyak Ramyeon versi instan yang bisa kamu beli di supermarket Indonesia."
    },
    {
      id: 8,
      name: 'Jjajangmyeon',
      category: 'korea',
      stock: 12,
      image:
        'https://www.selasar.com/wp-content/uploads/2020/09/Jjajangmyeon-Makanan-Khas-Korea.jpeg',
      price: 32000,
      description: "Jajangmyeon banyak ditemukan di Indonesia dan menjadi salah satu makanan khas Korea yang halal. Makanan ini berupa yang dibumbui saus kedelai hitam sehingga tampilannya berwarna hitam. Di dalamnya juga ada aneka jenis sayuran dan potongan daging dengan cita rasa gurih manis."
    },
    {
      id: 9,
      name: 'Pisang Goreng',
      category: 'indo',
      stock: 40,
      image:
        'https://media.istockphoto.com/id/1449091099/id/foto/sepiring-pisang-goreng-tepung-indonesia-atau-camilan-pisang-goreng-untuk-sarapan-atau.webp?s=1024x1024&w=is&k=20&c=klJNVYCHvj59BzUMMhjSUQlDJF2Jhmd2dNoHSKUFJgg=',
      price: 2000,
      description: "Pisang goreng merupakan makanan ringan yang banyak di temukan di Indonesia, bahkan mungkin di negara lain. Pisang goreng juga merupakan salah satu jenis gorengan yang digemari oleh masyarakat Indonesia. Terutama saat musim hujan seperti sekarang ini dan sangat cocok menjadi teman terbaik buat ngopi maupun ngeteh bersama doi."
    },
    {
      id: 10,
      name: 'Bibimbap',
      category: 'korea',
      stock: 14,
      image:
        'https://www.selasar.com/wp-content/uploads/2020/09/Bibimbap-Makanan-Khas-Korea.jpg',
      price: 40000,
      description: "Bibimbap adalah masakan Korea berupa semangkuk nasi putih dengan lauk di atasnya berupa sayur-sayuran, daging sapi, telur, dan saus pedas gochujang. Namanya secara harafiah berarti nasi campur yang berasal dari kata 비빔 (campur) dan 밥 (nasi). Sebelum dimakan, nasi dan lauk diaduk menjadi satu. Bibimbap memiliki variasi yang banyak menurut daerahnya di Korea."
    },
    {
      id: 11,
      name: 'Sukiyaki',
      category: 'jepang',
      stock: 9,
      image:
        'https://images.unsplash.com/photo-1648977555545-4dd006e30d3f?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1170&q=80',
      price: 28000,
      description: "Sukiyaki  jadi makanan khas Jepang yang terdiri dari daging dan sayuran yang direbus ke dalam sebuah panci. Untuk menciptakan rasa yang sangat gurih dan manis, dibutuhkan bumbu khusus yaitu warishita yang terbuat dari kecap asin dan tambahan gula. Di beberapa daerah di Jepang, menikmati sukiyaki selalu ditambahkan dengan kocokan telur ke dalam bumbu supaya membuat rasa menjadi lebih ringan di mulut."
    },
    {
      id: 12,
      name: 'Jjamppong',
      category: 'korea',
      stock: 7,
      image:
        'https://www.selasar.com/wp-content/uploads/2020/09/Jjamppong-Makanan-Khas-Korea.jpeg',
      price: 38000,
      description: "Makanan yang sehat dari Korea ini mempunyai isian cukup lengkap seperti udang, kerang, cumi, gurita dan lainnya. Selain aneka seafood, jjamppong juga dilengkapi dengan aneka sayur. Dalam semangkuk mie penuh dengan berbagai isian yang tentunya sangat mengenyangkan."
    },
    
    {
      id: 13,
      name: 'Nasi Liwet',
      category: 'indo',
      stock: 15,
      image:
        'https://th.bing.com/th/id/OIP.-CJdqHcvvUcffc0PDy2iLAHaE6?pid=ImgDet&rs=1',
      price: 20000,
      description: "Nasi liwet adalah hidangan nasi khas Indonesia yang dimasak dengan santan, kaldu ayam, dan rempah-rempah Nasi kukus biasanya dimasak dalam air, tetapi nasi liwet adalah nasi yang dimasak dengan santan, kaldu ayam, daun salam dan serai, sehingga memberikan nasi rasa yang kaya, aromatik, dan gurih. Nasi liwet adalah cara memasak nasi tradisional Jawa dengan santan. Ada juga varian nasi liwet ala Sunda yang berbeda rasa dan penyajiannya."
    },
    {
      id: 14,
      name: 'Kimchi',
      category: 'korea',
      stock: 11,
      image:
        'https://www.selasar.com/wp-content/uploads/2020/09/Kimchi-Makanan-Khas-Korea.jpg',
      price: 45000,
      description: "Kimchi dibuat dari sayuran yang difermentasi dan dibumbui sehingga memiliki cita rasa asam pedas. Biasanya makanan ini dijadikan sebagai pelengkap sajian utama di resto resto Korea. Bagi masyarakat Korea sendiri, rasanya kurang nikmati jika menyantap mie instan tanpa kimchi."
    },
    {
      id: 15,
      name: 'Onigiri',
      category: 'jepang',
      stock: 30,
      image:
        'https://cdn.idntimes.com/content-images/community/2021/01/images-42-cad2b25a01dc8f98bd8ca892872337a4-095233fbd8ddcc63098a7a4f1d40bf0e_600x400.jpeg',
      price: 12000,
      description: "Kudapan yang dibuat dari bahan dasar nasi ini dibentuk menjadi segitiga dan bola-bola. Sebelum dibentuk, nasi dicampur dengan sedikit garam serta dibungkus dengan lembaran nori atau rumput  laut kering. Nasi ini juga banyak yang diisi dengan potongan daging salmon dan telur ikan sebelum akhirnya dibungkus memakai nori."
    },
    {
      id: 16,
      name: 'Oden',
      category: 'korea',
      stock: 22,
      image:
        'https://www.selasar.com/wp-content/uploads/2020/09/Oden-Makanan-Khas-Korea.jpg',
      price: 10000,
      description: "Jajanan Korea ini terbuat dari ikan yang bentuknya mirip seperti pempek Indonesia. Apalagi proses pembuatannya diolah dengan tepung lalu digoreng. Hanya saja tekstur oden ini lebih tipis, lalu ditusuk dengan tusukan sate. Banyak masyarakat Korea yang memakan oden dengan cara dicampur dalam tteokbokki atau ramyeon. Biasanya oden dijual dalam truk truk makanan pinggir jalan di Korea."
    },
    {
      id: 17,
      name: 'Sashimi',
      category: 'jepang',
      stock: 9,
      image:
        'https://cms.sehatq.com/public/img/article_img/perbedaan-sushi-dan-sashimi-apa-saja-1632371532.jpg',
      price: 43000,
      description: "Kuliner yang satu ini menyajikan irisan daging ikan mentah dalam potongan kecil dan bisa langsung dimakan. Jadi tidak dimasak terlebih dahulu seperti memasak ikan pada umumnya. Sashimi akan terasa nikmat jika disantap dengan kecap asin dan tambahan seperti wasabi dan asinan jahe."
    },
    {
      id: 18,
      name: 'Pempek',
      category: 'indo',
      stock: 17,
      image:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/c/ca/Pempek_Kuah_Cuko.jpg/375px-Pempek_Kuah_Cuko.jpg',
      price: 24000,
      description: "Pempek atau empek-empek adalah makanan yang terbuat dari daging ikan yang digiling lembut yang dicampur tepung kanji atau tepung sagu, serta komposisi beberapa bahan lain seperti telur, bawang putih yang dihaluskan, penyedap rasa, dan garam. Pempek biasanya disajikan dengan kuah yang disebut cuka yang memiliki rasa asam, manis, dan pedas."
    },
    {
      id: 19,
      name: 'Udon',
      category: 'jepang',
      stock: 5,
      image:
        'https://asset.kompas.com/crops/Uu1b_Q-dLyJeagjsVJX6nOT65HQ=/0x0:699x466/780x390/data/photo/2020/08/23/5f41e3ce61b03.jpg',
      price: 36000,
      description: "Udon merupakan salah satu jenis mie Jepang yang dibuat dari tepung terigu. Warnanya putih cerah dengan diameter 1½ cm dan kenyal teksturnya. Dibandingkan mi biasa, udon lebih liat dan kenyal. Biasanya diolah dengan kuah bening dari kaldu dashi atau shoyu atau miso. Udon umumnya dijual dalam keadaan segar atau beku."
    },
    {
      id: 20,
      name: 'Soba',
      category: 'jepang',
      stock: 13,
      image:
        'https://asset.kompas.com/crops/UQ4qAfHmle-adBp5P6Tme3O-Sh4=/0x0:1000x667/750x500/data/photo/2018/09/29/650777653.jpg',
      price: 23000,
      description: "Banyak sekali kuliner berbahan dasar mi selain ramen di Jepang. Mie khas lainnya yang patut dicoba adalah soba. Menu ini makin nikmat disantap jika dilengkapi dengan topping seperti potongan telur, tempura, dan bahan-bahan lainnya.  Soba sangat populer di Jepang dan mudah didapati di banyak tempat makan kaki lima hingga restoran."
    },
    {
      id: 21,
      name: 'Gado-gado',
      category: 'indo',
      stock: 28,
      image:
        'https://www.selasar.com/wp-content/uploads/2020/08/gado-gado-makanan-khas-indonesia-.jpg',
      price: 22000,
      description: "Menu khas ini memiliki beberapa komposisi dari kangkung, bayam, wortel, kubis, selada, dan pare. Kemudian, sayuran-sayuran tersebut akan disajikan dengan lontong, tempe goreng, serta tahu goreng. Umumnya, seorang yang vegetarian akan menyukai makanan ini."
    },
    {
      id: 22,
      name: 'Bulgogi',
      category: 'korea',
      stock: 8,
      image:
        'https://www.selasar.com/wp-content/uploads/2020/09/Bulgogi-Makanan-Khas-Korea.jpg',
      price: 22000,
      description: "Dari gambarnya saja sepertinya makanan korea ini cukup menggoda lidah ya. Bahan utamanya berupa potongan daging sapi yang diiris tipis dan dibumbui sehingga menghasilkan cita rasa yang sangat lezat. Dalam proses pemasakannya, jika daging sapi sudah matang akan diberi taburan wijen di atasnya."
    },
    {
      id: 23,
      name: 'Soto',
      category: 'indo',
      stock: 20,
      image:
        'https://www.selasar.com/wp-content/uploads/2020/08/soto-makanan-khas-indonesia-.jpg',
      price: 22000,
      description: "Soto adalah makanan khas Indonesia dengan kuah yang terbuat dari kaldu daging dan sayuran. Umumnya, daging untuk isian Soto ini di antaranya adalah daging ayam dan daging sapi."
    },
    {
      id: 24,
      name: 'Takoyaki',
      category: 'jepang',
      stock: 12,
      image:
        'https://makananoleholeh.com/wp-content/uploads/2018/09/Takoyaki-Jepang.jpg',
      price: 22000,
      description: "Takoyaki merupakan salah satu makanan khas jepang yang sangat terkenal dan wajib dicicipi apabila anda sedang berkunjung ke jepang. Bahan dasar dari makanan ini adalah tepung terigu yang diberi isian gurita. Rasa dari makanan ini sangat lezat karena selain berisi daging gurita juga akan ada banyak isian lainnya seperti keju, cumi, udang dan yang lainnya. Anda bisa memilih takoyaki dengan isian kesukaan anda."
    },
  ];
  
  export const kategori = [
    { name: "Indonesia", categories: "indo" },
    { name: "Jepang", categories: "jepang" },
    { name: "Korea", categories: "korea" },
  ]

  export const harga = [
    { harga1: 0, harga2: 10000, display: "< 10.000" },
    { harga1: 10001, harga2: 20000, display: "10.000-20.000" },
    { harga1: 20001, harga2: 30000, display: "20.000-30.000" },
    { harga1: 30001, harga2: 100000, display: "> 30.000" },
  ]
  export const testi = [
    {
      nama: "Rainnnn",
      title: "Classmate",
      testi1: "Sebagai anak kos, aku sering pakai ini. ",
      mark: "User-friendly dan fleksibel",
      testi2:
        "menjadi salah satu alasan aku selalu menggunakannya.",
      img: "/images/rain.JPG",
    },
    {
      nama: "Risha Luffy",
      title: "Classmate",
      testi1: "Ini adalah ecommerce website yang ",
      mark: "keren",
      testi2:
        ". Aku akan terus menggunakannya untuk memesan makanan.",
      img: "/images/risha.jpeg",
    },
    {
      nama: "Kafanal Kafi",
      title: "Classmate",
      testi1: "Overall ",
      mark: "keren",
      testi2: ", tapi aku kurang suka dengan deskripsi yang terlalu panjang dan kurang efisien di bagian detailnya.",
      img: "/images/kafi.jpg",
    },
  ];